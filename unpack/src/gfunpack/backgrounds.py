...
class BackgroundCollection:
    ...
    def _save_images(self, all_images: Dict[str, Union[Sprite, Texture2D]]) -> Dict[str, Path]:
        saved_images = {}
        args = [((name, img), self.output_path, self.pngquant, self.force) for name, img in all_images.items()]
        ...
        with Pool(processes=self.concurrency) as pool:
            ...
            saved_images = {name: path for (name, _), path in zip(all_images.items(), results) if path is not None}
        return saved_images
